var binding = require("./node_printer.node");
 ...
 */
module.exports.getSupportedPrintFormats = binding.getSupportedPrintFormats;

/**
 * Get possible job command for setJob. It depends on os.
 * @return Array of string. e.g.: DELETE, PAUSE, RESUME
 */
module.exports.getSupportedJobCommands = binding.getSupportedJobCommands;
...
function getDefaultPrinterName() {
  var printerName = binding.getDefaultPrinterName();
  if (printerName) {
    return printerName;
  }
...
function getPrinter(printerName) {
  ...
  var printer = binding.getPrinter(printerName);
...
function getPrinterDriverOptions(printerName) {
  ...
  return binding.getPrinterDriverOptions(printerName);
}
...
function getJob(printerName, jobId) {
  return binding.getJob(printerName, jobId);
}

function setJob(printerName, jobId, command) {
  return binding.setJob(printerName, jobId, command);
}

function getPrinters() {
  var printers = binding.getPrinters();
  if (printers && printers.length) {
    var i = printers.length;
    for (i in printers) {
...
function printDirect(parameters) {
  ...
  if (binding.printDirect) {
    // call C++ binding
    try {
      var res = binding.printDirect(data, printer, docname, type, options);
      if (res) {
        success(res);
      } else {
...
function printFile(parameters) {
  var filename, docname, printer, options, success, error;
  ...
  if (!printer) {
    return error(
      new Error("Printer parameter of default printer is not defined")
    );
  }
  ...
  if (binding.printFile) {
    ...
    try {
      ...
      var res = binding.printFile(filename, docname, printer, options);
